<div class="card">
    <div class="card-header">
        <h3 class="card-title">Edit Grade {{$myClass->name}}</h3>
    </div>
    <div class="card-body">
        <form action="{{route('classes.update', $myClass->id)}}" method="POST" class="md:w-6/12">
            <x-display-validation-errors/>
            <x-input id="edit-class-name" name="name" label="Grade Name" placeholder="Enter grade name" fgroup-class="" value="{{$myClass->name}}"/>
            <x-select id="edit-class-class_group_id" name="class_group_id" class=" m-2" label="Level">
