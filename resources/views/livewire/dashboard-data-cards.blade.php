        ...
        <div class="card-body">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                @can('read class group')
                    <x-info-box title="{{$classGroups}}" text="Levels" colour="bg-orange-600"  text-colour="text-white" url="{{route('class-groups.index')}}" url-text="View Levels"/>
                @endcan
                @can('read class')
                    <x-info-box title="{{$classes}}" text="Grades" url="{{route('classes.index')}}" url-text="View Grades" colour="bg-green-500"  text-colour="text-white"/>
                @endcan
                @can('read section')
                    <x-info-box title="{{$sections}}" text="Streams" url="{{route('sections.index')}}" url-text="View Streams" colour="bg-lime-600"  text-colour="text-white" />
                @endcan
                @can('read student')
